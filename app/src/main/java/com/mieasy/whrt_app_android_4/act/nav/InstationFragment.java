...
//	private Matrix matrix = new Matrix();
	...
	public View onCreateView(LayoutInflater inflater,ViewGroup container, Bundle savedInstanceState) {
		view = inflater.inflate(R.layout.map_nav_instation, null);
...
//				mImageView.setImageBitmap(bmp);
	...
	private void oneShow(){
		...
		if(directions.size()!=0){
			...
			if (timeTable0.size()>0) tvWorkingStartOneOfT.setText(timeTable0.get(0).getWeekdayFirst()+"-"+timeTable0.get(0).getWeekdayLast());
			if (timeTable0.size()>0) tvSundayStopOneOfT.setText(timeTable0.get(0).getWeekendFirst()+"-"+timeTable0.get(0).getWeekendLast());
			if (timeTable1.size()>0) tvWorkingStartOneOfF.setText(timeTable1.get(0).getWeekdayFirst()+"-"+timeTable1.get(0).getWeekdayLast());
			if (timeTable1.size()>0) tvSundayStopOneOfF.setText(timeTable1.get(0).getWeekendFirst()+"-"+timeTable1.get(0).getWeekendLast());
		}
	}
	...
	private void twoShow(){
		...
		//是换乘站
		tvStationName.setText(stations.getStationName()+"换乘站");
		...
		if(stationsList.size()>=2){
			...
			if(directions1.size()!=0){
				...
				if (timeTable0.size()>0) tvWorkingStartTwoOfT.setText(timeTable0.get(0).getWeekdayFirst()+"-"+timeTable0.get(0).getWeekdayLast());
				if (timeTable0.size()>0) tvSundayStopTwoOfT.setText(timeTable0.get(0).getWeekendFirst()+"-"+timeTable0.get(0).getWeekendLast());
				if (timeTable1.size()>0) tvWorkingStartTwoOfF.setText(timeTable1.get(0).getWeekdayFirst()+"-"+timeTable1.get(0).getWeekdayLast());
				if (timeTable1.size()>0) tvSundayStopTwoOfF.setText(timeTable1.get(0).getWeekendFirst()+"-"+timeTable1.get(0).getWeekendLast());
			}
		}else {
			oneShow();
		}
	...
	public void checkImageView(ImageView imgView,TextView tvTxt,int i){
		if(i==1){
			...
			imgView.setImageResource(R.drawable.line_no06);
			tvTxt.setText("轨道交通六号线");
		}
	}
