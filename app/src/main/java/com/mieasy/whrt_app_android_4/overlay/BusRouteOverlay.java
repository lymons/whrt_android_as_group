package com.mieasy.whrt_app_android_4.overlay;
...
public class BusRouteOverlay extends RouteOverlay {
	...
	public BusRouteOverlay(Context context, AMap amap, BusPath path,
		...
		super(context);
		this.busPath = path;
		startPoint = AMapStationUtil.convertToLatLng(start);
		endPoint = AMapStationUtil.convertToLatLng(end);
		mAMap = amap;
	}
		...
		try {
			...
			for (int i = 0; i < busSteps.size(); i++) {
				...
				if (busStep.getBusLine() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(AMapStationUtil.convertToLatLng(getLastBuslinePoint(busStep)), endPoint);
					}
				}
				if (busStep.getRailway() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(AMapStationUtil.convertToLatLng(busStep.getRailway().getArrivalstop().getLocation()), endPoint);
					}
				}
	...
	private void addRailwayStep(RouteRailwayItem railway) {
		...
		for (int i = 0; i < railwayStationItems.size(); i++) {
			railwaylistpoint.add(AMapStationUtil.convertToLatLng(railwayStationItems.get(i).getLocation()));
		}
	...
	private void addTaxiStep(TaxiItem taxi){
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.add(AMapStationUtil.convertToLatLng(taxi.getOrigin()))
				.add(AMapStationUtil.convertToLatLng(taxi.getDestination())));
	}
	...
	private void addWalkSteps(BusStep busStep) {
		...
		for (int j = 0; j < walkSteps.size(); j++) {
			...
			if (j == 0) {
				LatLng latLng = AMapStationUtil.convertToLatLng(walkStep
						.getPolyline().get(0));
			...
			List<LatLng> listWalkPolyline = AMapStationUtil
			...
			if (j < walkSteps.size() - 1) {
				LatLng lastLatLng = listWalkPolyline.get(listWalkPolyline
						.size() - 1);
				LatLng firstlatLatLng = AMapStationUtil
	...
	private void addBusLineSteps(List<LatLonPoint> listPoints) {
		...
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.addAll(AMapStationUtil.convertArrList(listPoints)));
	}
	...
	private void addBusStationMarkers(RouteBusLineItem routeBusLineItem) {
		...
		LatLng position = AMapStationUtil.convertToLatLng(startBusStation
				.getLatLonPoint());
	...
	private void addTaxiMarkers(TaxiItem taxiItem) {
		...
		LatLng position = AMapStationUtil.convertToLatLng(taxiItem
				.getOrigin());
	...
	private void addRailwayMarkers(RouteRailwayItem railway) {
		LatLng Departureposition = AMapStationUtil.convertToLatLng(railway
		...
		LatLng Arrivalposition = AMapStationUtil.convertToLatLng(railway
	...
	private void checkBusToNextBusNoWalk(BusStep busStep, BusStep busStep1) {
		LatLng endbusLatLng = AMapStationUtil
				.convertToLatLng(getLastBuslinePoint(busStep));
		LatLng startbusLatLng = AMapStationUtil
	...
	private void checkBusEndToNextBusStart(BusStep busStep, BusStep busStep1) {
		LatLonPoint busLastPoint = getLastBuslinePoint(busStep);
		LatLng endbusLatLng = AMapStationUtil.convertToLatLng(busLastPoint);
		LatLonPoint busFirstPoint = getFirstBuslinePoint(busStep1);
		LatLng startbusLatLng = AMapStationUtil.convertToLatLng(busFirstPoint);
	...
	private void addWalkPolyLineByLatLonPoints(LatLonPoint pointFrom,
			LatLonPoint pointTo) {
		LatLng latLngFrom = AMapStationUtil.convertToLatLng(pointFrom);
		LatLng latLngTo = AMapStationUtil.convertToLatLng(pointTo);
